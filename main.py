...
from gpt_utils import AsyncGPT3ProbBackendHighTemp, AsyncChatQueue
...
async def main():
    ...
    users = [(
        f"Party 1: {row['origname_1']} "
        f"Party 1 country: {row['country_1']} "
        f"Party 2: {row['origname_2']} "
        f"Party 2 country: {row['country_2']} "
    ) for index, row in dup.iterrows()]

    chats = [AsyncGPT3ProbBackendHighTemp(system, user) for user in users]
